...
parser = argparse.ArgumentParser(description="Run Grad-CAM explainability for CharBiLSTM model.")
...
def gradcam_charbilstm(model, input_tensor, layer_name="bidirectional", class_idx=None):
    """
    Compute Grad-CAM over a BiLSTM layer.
    """
    grad_model = tf.keras.models.Model([model.input], [model.get_layer(layer_name).output, model.output])
    with tf.GradientTape() as tape:
        lstm_output, predictions = grad_model(input_tensor)
        if class_idx is None:
            class_idx = tf.argmax(predictions[0])
        class_output = predictions[:, class_idx]

    grads = tape.gradient(class_output, lstm_output)[0]
    weights = tf.reduce_mean(grads, axis=0)
    cam = tf.reduce_sum(tf.multiply(weights, lstm_output[0]), axis=-1)
...
def export_classified_html(results_df, input_size, filename, condition, color_scheme):
    ...
    if subset.empty:
        print(f"No matching samples for {filename}")
        return
...
for i, row in df_test_sample.iterrows():
    ...
    try:
        ...
        cam, confidence = gradcam_charbilstm(model, input_tensor, layer_name="bidirectional", class_idx=label)
