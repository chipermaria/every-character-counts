...
if __name__ == "__main__":
    ...
    # Apply DeepWordBug-style attack to 40% of phishing training samples
    phishing_train_idx = df_train[df_train["labels"] == 1].sample(frac=0.4, random_state=42).index
    df_train.loc[phishing_train_idx, "contents"] = df_train.loc[phishing_train_idx, "contents"].apply(
        lambda x: deepwordbug_attack(x, perturbation_rate=0.2, alphabet=attack_alphabet)
    )
    ...
    model = CharGRU(
        ...
        emb_sz=config["chargru"]["embedding_size"],
        gru_units=config["chargru"]["gru_units"],
        dropout_p=config["chargru"]["dropout_p"],
        optimizer=config["chargru"]["optimizer"],
        loss=config["chargru"]["loss"]
    )
    ...
    model.save("adv_train_test_model.h5")
